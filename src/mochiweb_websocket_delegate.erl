...
process_data(State = #state{buffer= <<Len:64/unsigned-integer,Buffer/binary>>, ft=255, flen=undefined}) ->
    ...
    case Buffer of
        <<Frame:BitsLen/binary, Rest/binary>> ->            
            State#state.dest ! {websockets_frame, Frame},
...
websocket_frame_parser_test() ->
    ...
    {websockets_frame, <<"what on earth is a quaid?">>} = receive_once(),
    {websockets_frame, <<"the quick brown fox jumps over the lazy dog">>} = receive_once(),
    {websockets_frame, <<"and what's so good about smints?">>} = receive_once(),
